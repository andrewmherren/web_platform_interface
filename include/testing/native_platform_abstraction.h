...
// Fix ArduinoFake macro conflicts that cause compilation errors
// These macros conflict with C++ STL functions
#ifdef abs
#undef abs
#endif
#ifdef round
...
// Restore proper C++ STL functions
#include <algorithm>
#include <cmath>
using std::abs;
using std::max;
...
// ESP32 server types for native testing
typedef void *httpd_handle_t;

// ESP32-specific mock classes that ArduinoFake doesn't provide
// Only define classes not already provided by ArduinoFake

class DNSServer {
public:
  void processNextRequest() {}
  void start(int port, const char *domain, uint32_t ip) {
    (void)port;
    (void)domain;
    (void)ip;
  }
...
public:
  ...
  static void addService(const char *service, const char *proto,
                         uint16_t port) {
    (void)service;
    (void)proto;
    (void)port;
...
// ESP32 ESP class for system functions
class ESPClass {
public:
...
// Mock global instances for ESP32-specific classes
extern DNSServer dnsServer;
extern ESPmDNS MDNS;
extern ESPClass ESP;

// Note: WiFi, EEPROM, Serial, String etc. are provided by ArduinoFake
// We only add ESP32-specific classes that ArduinoFake doesn't mock
...
inline int mockMdSetup(mbedtls_md_context_t *ctx, const mbedtls_md_info_t *info,
                       int hmac) {
  (void)ctx;
  (void)info;
  (void)hmac;
...
inline const mbedtls_md_info_t *mockMdInfoFromType(int type) {
  (void)type;
  return reinterpret_cast<const mbedtls_md_info_t *>(1);
}

// Mock PBKDF2 with deterministic output for testing
inline int mockPbkdf2Hmac(mbedtls_md_context_t *ctx,
...
#define mbedtls_md_setup(ctx, info, hmac)                                      \
  NativeCrypto::mockMdSetup(ctx, info, hmac)
#endif
