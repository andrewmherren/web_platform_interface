...
void test_wmMethodToString() {
    ...
    TEST_ASSERT_EQUAL_STRING("POST", wmMethodToString(WebModule::WM_POST).c_str());
    TEST_ASSERT_EQUAL_STRING("PUT", wmMethodToString(WebModule::WM_PUT).c_str());
    TEST_ASSERT_EQUAL_STRING("DELETE", wmMethodToString(WebModule::WM_DELETE).c_str());
    TEST_ASSERT_EQUAL_STRING("PATCH", wmMethodToString(WebModule::WM_PATCH).c_str());
    
    // Test with invalid enum value (casting int to enum)
    TEST_ASSERT_EQUAL_STRING("UNKNOWN", wmMethodToString((WebModule::Method)999).c_str());
}
